<?php
// Heading
$_['heading_title']      = 'Klarna Invoice';

// Text
$_['text_payment']          = 'Оплата';
$_['text_success']          = 'Настройки модуля обновлены!';
$_['text_klarna_invoice']   = '<a href=https://merchants.klarna.com/signup?locale=en&partner_id=d5c87110cebc383a826364769047042e777da5e8&utm_campaign=Platform&utm_medium=Partners&utm_source=Opencart target=_blank><img src=https://cdn.klarna.com/public/images/global/logos/v1/basic/global_basic_logo_std_blue-black.png?width=60&eid=opencart alt=Klarna Invoice title=Klarna Invoice style=border: 1px solid #EEEEEE; /></a>';
$_['text_live']             = 'Live';
$_['text_beta']             = 'Beta';
$_['text_sweden']           = 'Швеция';
$_['text_norway']           = 'Норвегия';
$_['text_finland']          = 'Финляндия';
$_['text_denmark']          = 'Дания';
$_['text_germany']          = 'Германия';
$_['text_netherlands']      = 'Нидерланды';

// Entry
$_['entry_merchant']        = 'Klarna Merchant ID:(estore id) для использования сервиса (предоставляется Klarna).';
$_['entry_secret']          = 'Klarna Secret:Секретный ключ для использования сервиса (предоставляется Klarna).';
$_['entry_server']          = 'Сервер:';
$_['entry_total']           = 'Минимальная сумма заказа:Сумма заказа, после достижения которой данный способ станет доступен.';
$_['entry_pending_status']  = 'Статус при ожидании:';
$_['entry_accepted_status'] = 'Статус принятого платежа:';
$_['entry_geo_zone']        = 'Географическая зона:';
$_['entry_status']          = 'Статус:';
$_['entry_sort_order']      = 'Порядок сортировки:';

// Error
$_['error_permission']    = 'У Вас нет прав для управления этим модулем!';
?>